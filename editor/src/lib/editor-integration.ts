...
export class EditorIntegration {
  ...
  async processLegalCommand(command: string): Promise<LegalEditResponse | null> {
    try {
      ...
      const extractedContent = JSONExtractor.extractSelectedContent(this.editor);
      
      if (!extractedContent) {
        throw new Error('No content selected. Please select text to edit.');
      }

      // Validate extracted content
      const validation = JSONExtractor.validateExtractedJSON(extractedContent.jsonSlice);
      if (!validation.isValid) {
        throw new Error(`Invalid selected content: ${validation.errors.join(', ')}`);
      }
