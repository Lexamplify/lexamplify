...
4. Each search query should be 5-10 words optimized for case law search with these guidelines:
   - Include specific legal terms like "Section 302", "Indian Penal Code", "evidence", "constitutional rights"
   - Use landmark case names when relevant (e.g., "Virsa Singh murder")
   - Include procedural terms like "fair trial", "due process", "reasonable doubt"
   - Focus on the core legal issue being argued
5. If no respondent arguments found, use empty array: "respondentArgs": []

Examples of good search queries:
- "Section 302 murder interpretation Supreme Court"
- "insufficient evidence criminal trial conviction"
- "constitutional rights fair trial violation"
- "prosecution evidence beyond reasonable doubt"
- "lower court error interpretation law"
...
const findCitationsForArgumentsFlow = ai.defineFlow(
  ...
  async (input) => {
    ...
    const petitionerSearchTasks = extractedData.petitionerArgs.map(async (arg: { argument: string; searchQuery: string }) => {
      ...
      // Define comprehensive predefined citations for common legal arguments
      const predefinedCitations: Record<string, Array<{title: string, url: string}>> = {
        // Section 302 interpretation errors
      ...
      // Try to match the argument with predefined categories using flexible matching
      ...
      // If no exact match, try partial matching with common legal terms
      if (!matched) {
        const legalTerms = ['section 302', 'evidence', 'constitutional', 'fair trial', 'due process', 'violated', 'erred', 'insufficient', 'reasonable doubt'];
        for (const term of legalTerms) {
          if (lowerArg.includes(term)) {
      ...
      // If no predefined citations found, try to search with multiple query strategies
      if (citations.length === 0) {
        console.log(`[Search] No predefined citations found for: "${arg.argument}"`);
        
        // Try multiple search strategies
        const searchQueries = [
          arg.searchQuery,
          arg.argument.substring(0, 50), // First 50 chars of argument
          `Indian Penal Code ${arg.argument.substring(0, 30)}`, // Add IPC context
          `criminal law ${arg.argument.substring(0, 30)}` // Add criminal law context
        ];
        
        for (const query of searchQueries) {
          if (query.trim()) {
            console.log(`[Search] Trying query: "${query}"`);
            const searchResults = await indianKanoonSearch.run({ query: query.trim() });
            // Handle the ActionResult type properly
            if (searchResults && searchResults.result && Array.isArray(searchResults.result) && searchResults.result.length > 0) {
              citations = searchResults.result.slice(0, 2).map((result: { title: string; url: string }) => ({
                title: result.title,
                url: result.url
              }));
              console.log(`[Search] Found ${citations.length} citations with query: "${query}"`);
    ...
    const respondentSearchTasks = extractedData.respondentArgs
      ? extractedData.respondentArgs.map(async (arg: { argument: string; searchQuery: string }) => {
          ...
          // Define comprehensive predefined citations for respondent arguments
          const predefinedCitations: Record<string, Array<{title: string, url: string}>> = {
          ...
          // If no exact match, try partial matching with common legal terms
          if (!matched) {
            const legalTerms = ['section 302', 'evidence', 'constitutional', 'due process', 'prosecution', 'prove', 'correct', 'sufficient', 'mitigating', 'adherence'];
            for (const term of legalTerms) {
              if (lowerArg.includes(term)) {
                // Find the best matching predefined category
                for (const [key, value] of Object.entries(predefinedCitations)) {
          ...
          // If no predefined citations found, try to search with multiple query strategies
          if (citations.length === 0) {
            console.log(`[Search] No predefined citations found for respondent arg: "${arg.argument}"`);
            ...
            for (const query of searchQueries) {
              if (query.trim()) {
                console.log(`[Search] Trying respondent query: "${query}"`);
                const searchResults = await indianKanoonSearch.run({ query: query.trim() });
                // Handle the ActionResult type properly
                if (searchResults && searchResults.result && Array.isArray(searchResults.result) && searchResults.result.length > 0) {
                  citations = searchResults.result.slice(0, 2).map((result: { title: string; url: string }) => ({
                  ...
                  console.log(`[Search] Found ${citations.length} citations with respondent query: "${query}"`);
                  break; // Stop at first successful search
                }
              }
            }
